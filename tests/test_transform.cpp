...
TEST(TransformTest, Translation) {
    ...
    const auto t = transform::Transform2D::translation(3, 4);

    auto p2 = geometry::normalize(t.H * p);
...
TEST(TransformTest, Scale) {
    ...
    const auto s = transform::Transform2D::scale(2);

    auto p2 = geometry::normalize(s.H * p);
...
TEST(TransformTest, Rotation90Deg) {
    ...
    const auto r = transform::Transform2D::rotation(M_PI / 2);

    auto p2 = geometry::normalize(r.H * p);
...
TEST(TransformTest, Composition) {
    ...
    const auto t = transform::Transform2D::translation(1, 0);

    const auto s = transform::Transform2D::scale(2);

    auto p2 = geometry::normalize(s.H * (t.H * p));
