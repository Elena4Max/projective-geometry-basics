...
double projectionDifference(
    const core::Vec3& X,
    const camera::CameraIntrinsics& referenceK,
    ...
    const camera::CameraExtrinsics& E)
{
    ...
    return geometry::euclideanDistance(
        reference,
        perturbed);
}

static std::vector<cv::Point3f> createChessboardPoints(
    int cols,
    int rows,
    float squareSize)
{
...
int main(int argc, char** argv)
{
    if (argc != 2) {
        std::cerr
            << "Usage: ./visualize_calibration <images_dir>\n";
        return 1;
    }
    ...
    for (std::size_t i = 0; i < objectPoints.size(); ++i)
    {
            ...
            ++j)
        {
            ...
            const auto customProjected =
                camera::projectPoint(
                    X,
                    K,
                    E);

            geometry::Point2D cvPoint{
                cvProjectedPoints[j].x,
                cvProjectedPoints[j].y,
                1.0
            };
    ...
    camera::CameraExtrinsics E{
        ...
        core::Vec3{
            tvecs.front().at<double>(0),
            tvecs.front().at<double>(1),
    ...
    const auto image =
    cv::imread(imagePaths.front().string());
    ...
    cv::imwrite(
        (outputDir / "reprojection_validation.png")
            .string(),
        vis);
