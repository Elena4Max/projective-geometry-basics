...
cv::Point2d toCartesian(geometry::Point2D& p) { return {p.x / p.z, p.y / p.z}; }

void drawText(cv::Mat& img, const std::string& text, int x, int y) {
    cv::putText(img, text, {x, y}, cv::FONT_HERSHEY_SIMPLEX, 0.6, {0, 0, 0}, 1);
}
...
void drawPoint(cv::Mat& img, const geometry::Point2D& p, const cv::Scalar& color, int offsetX = 0) {
    ...
    cv::circle(img, {offsetX + (int)pn.x, (int)pn.y}, 5, color, -1);
}
...
void drawLine(cv::Mat& img, const geometry::Line2D& l, const cv::Scalar& color, int offsetX = 0) {
    ...
    if (std::abs(l.y) > core::kEps) {
        ...
        cv::line(img, {offsetX + 0, (int)y1}, {offsetX + w, (int)y2}, color, 2);
    } else {
        double x = -l.z / l.x;
        cv::line(img, {offsetX + (int)x, 0}, {offsetX + (int)x, h}, color, 2);
    }
}
